...
if (isset($_GET['bed_id'])) {
    ...
    if ($bed) {
        if ($bed['student_id'] !== null && $bed['student_id'] !== '') {
            ...
            // Bed does not have a student ID, proceed with updating
            $updateQuery = "UPDATE beds SET is_occupied = 1 WHERE id = :bed_id";
            $updateStmt = $pdo->prepare($updateQuery);
            $updateStmt->execute([':bed_id' => $bedId]);

            header('Location: ../beds.php'); // Redirect back to the page
            exit();
        }
    } else {
        echo '<!DOCTYPE html>
